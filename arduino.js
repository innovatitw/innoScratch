...
(function(ext) {
  ...
  var blocks = {
    ...
    ja: [
      ...
      [' ', '%m.hwIn ?????入力 %n ??????', 'connectHW', 'rotation knob', 0],
      ['-'],
      [' ', '%m.leds ? %m.outputs ???', 'digitalLED', 'led A', 'on'],
      [' ', '%m.leds ?明??? %n% ???', 'setLED', 'led A', 100],
      [' ', '%m.leds ?明??? %n% ?????', 'changeLED', 'led A', 20],
      ['-'],
      [' ', '%m.servos ? %n 度?向??', 'rotateServo', 'servo A', 180],
      [' ', '%m.servos ? %n 度??回?', 'changeServo', 'servo A', 20],
      ['-'],
      ['h', '%m.buttons ? %m.btnStates ??', 'whenButton', '??? A', '押???'],
      ['b', '%m.buttons 押???', 'isButtonPressed', '??? A'],
      ['-'],
      ['h', '%m.hwIn ? %m.ops %n% ??????', 'whenInput', '回????', '>', 50],
      ['r', '%m.hwIn ??', 'readInput', '回????'],
      ['-'],
      [' ', '????出力 %n ? %m.outputs ???', 'digitalWrite', 1, 'on'],
      [' ', '????出力 %n ? %n% ???', 'analogWrite', 3, 100],
      ['-'],
      ['h', '????入力 %n ? %m.outputs ??????', 'whenDigitalRead', 1, 'on'],
      ['b', '????入力 %n ???', 'digitalRead', 1],
      ['-'],
      ['h', '????入力 %n ? %m.ops %n% ??????', 'whenAnalogRead', 1, '>', 50],
      ['r', '????入力 %n ??', 'analogRead', 0],
      ['-'],
      ['r', '%n ? %n ... %n ?? %n ... %n ??換', 'mapValues', 50, 0, 100, -240, 240]
    ],
    nb: [
