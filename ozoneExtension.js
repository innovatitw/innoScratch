...
(function(ext) {
  ...
  var blocks = {
    ...
    ja: [
      ['h', '?��??�ス?�つ?��?????��?', 'whenConnected'],
      [' ', '%m.hwOut ??%n ?�ン?�つ?��?', 'connectHW', 'led A', 3],
      [' ', '%m.hwIn ?�ア?�ロ?�入??%n ?�ン?�つ?��?', 'connectHW', 'rotation knob', 0],
      ['-'],
      [' ', '%m.leds ??%m.outputs ?��???, 'digitalLED', 'led A', 'on'],
      [' ', '%m.leds ?��??��???%n% ?��???, 'setLED', 'led A', 100],
      [' ', '%m.leds ?��??��???%n% ?�つ変�???, 'changeLED', 'led A', 20],
      ['-'],
      [' ', '%m.servos ??%n 度へ?��???, 'rotateServo', 'servo A', 180],
      [' ', '%m.servos ??%n 度�??��???, 'changeServo', 'servo A', 20],
      ['-'],
      ['h', '%m.buttons ??%m.btnStates ?��?', 'whenButton', '?�タ??A', '?��??��?'],
      ['b', '%m.buttons ?��??��?', 'isButtonPressed', '?�タ??A'],
      ['-'],
      ['h', '%m.hwIn ??%m.ops %n% ?�な????��?', 'whenInput', '?�転?�ま??, '>', 50],
      ['r', '%m.hwIn ?��?, 'readInput', '?�転?�ま??],
      ['-'],
      [' ', '?�ジ?�ル?��? %n ??%m.outputs ?��???, 'digitalWrite', 1, 'on'],
      [' ', '?��??�グ?��? %n ??%n% ?��???, 'analogWrite', 3, 100],
      ['-'],
      ['h', '?�ジ?�ル?��? %n ??%m.outputs ?�な????��?', 'whenDigitalRead', 1, 'on'],
      ['b', '?�ジ?�ル?��? %n ?�オ??, 'digitalRead', 1],
      ['-'],
      ['h', '?��??�グ?��? %n ??%m.ops %n% ?�な????��?', 'whenAnalogRead', 1, '>', 50],
      ['r', '?��??�グ?��? %n ?��?, 'analogRead', 0],
      ['-'],
    ...
    el: [
      ['h', '??αν η ???κε?ή είναι ??νδεδεμένη', 'whenConnected'],
      [' ', '??νδε?ε ?ο %m.hwOut ??ο pin %n', 'connectHW', 'led A', 3],
      [' ', '??νδε?ε ?ο %m.hwIn ??ο αναλογικ? %n', 'connectHW', '?ον?ε?ι?με??ο', 0],
      ['-'],
      [' ', 'άλλαξε ?ο %m.leds ?ε %m.outputs', 'digitalLED', 'led A', 'ενε?γο?οιημένο'],
      [' ', '??ι?ε ??ο %m.leds ?η ???ειν??η?α ί?η με %n%', 'setLED', 'led A', 100],
      [' ', 'άλλαξε ??ο %m.leds ?η ???ειν??η?α κα?ά %n%', 'changeLED', 'led A', 20],
      ['-'],
      [' ', '???ί?ε ?ο %m.servos ??ι? %n μοί?ε?', 'rotateServo', 'servo A', 180],
      [' ', '???ί?ε ?ο %m.servos κα?ά %n μοί?ε?', 'changeServo', 'servo A', 20],
      ['-'],
      ['h', '??αν ?ο %m.buttons είναι %m.btnStates', 'whenButton', 'κο?μ?ί A', '?α?ημένο'],
      ['b', '?ο %m.buttons ?α?ήθηκε;', 'isButtonPressed', 'κο?μ?ί A'],
      ['-'],
      ['h', '??αν ?ο %m.hwIn %m.ops %n%', 'whenInput', '?ον?ε?ι?με??ο', '>', 50],
      ['r', 'διάβα?ε %m.hwIn', 'readInput', '?ον?ε?ι?με??ο'],
      ['-'],
      [' ', 'άλλαξε ?ο pin %n ?ε %m.outputs', 'digitalWrite', 1, 'ενε?γο?οιημένο'],
      [' ', '??ι?ε ?ο pin %n ?ε %n%', 'analogWrite', 3, 100],
      ['-'],
      ['h', '??αν ?ο pin %n είναι %m.outputs', 'whenDigitalRead', 1, 'ενε?γο?οιημένο'],
      ['b', '?ο pin %n είναι ενε?γο?οιημένο;', 'digitalRead', 1],
      ['-'],
      ['h', '??αν ?ο αναλογικ? %n %m.ops %n%', 'whenAnalogRead', 1, '>', 50],
      ['r', 'διάβα?ε ?ο αναλογικ? %n', 'analogRead', 0],
      ['-'],
      ['r', '????έ?ι?ε %n α?? %n %n έ?? %n %n', 'mapValues', 50, 0, 100, -240, 240]
    ],
    es: [
  ...
  var menus = {
    ...
    ja: {
      buttons: ['?�タ??A', '?�タ??B', '?�タ??C', '?�タ??D'],
      btnStates: ['?��??��?', '?��??��?'],
      hwIn: ['?�転?�ま??, '?�セ?�サ??, '温度?�ン?�ー'],
      hwOut: ['led A', 'led B', 'led C', 'led D', '?�タ??A', '?�タ??B', '?�タ??C', '?�タ??D', '?�ー??A', '?�ー??B', '?�ー??C', '?�ー??D'],
      leds: ['led A', 'led B', 'led C', 'led D'],
      outputs: ['?�ン', '?��?'],
      ops: ['>', '=', '<'],
      servos: ['?�ー??A', '?�ー??B', '?�ー??C', '?�ー??D']
    },
    nb: {
    ...
    el: {
      buttons: ['κο?μ?ί A', 'κο?μ?ί B', 'κο?μ?ί C', 'κο?μ?ί D'],
      btnStates: ['?α?ημένο', 'ελε?θε?ο'],
      hwIn: ['?ον?ε?ι?με??ο', '???οαι?θη?ή?α', 'θε?μοαι?θη?ή?α'],
      hwOut: ['led A', 'led B', 'led C', 'led D', 'κο?μ?ί A', 'κο?μ?ί B', 'κο?μ?ί C', 'κο?μ?ί D', 'servo A', 'servo B', 'servo C', 'servo D'],
      leds: ['led A', 'led B', 'led C', 'led D'],
      outputs: ['ενε?γο?οιημένο', 'α?ενε?γο?οιημένο'],
      ops: ['>', '=', '<'],
      servos: ['servo A', 'servo B', 'servo C', 'servo D']
    },
    es: {
  ...
  var descriptor = {
    blocks: blocks[lang],
    menus: menus[lang],
    url: 'http://murphychiu.github.io/innoScrath'
  };
