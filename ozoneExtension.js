...
(function(ext) {
  ...
  var blocks = {
    ...
    de: [
      ...
      ['-'],
      ['h', 'Wenn %m.buttons ist %m.btnStates', 'whenButton', 'Taste A', 'gedr眉ckt'],
      ['b', '%m.buttons gedr眉ckt?', 'isButtonPressed', 'Taste A'],
      ['-'],
    ...
    it: [
      ['h', 'quando Arduino 猫 connesso', 'whenConnected'],
      ...
      ['-'],
      ['h', 'quando tasto %m.buttons 猫 %m.btnStates', 'whenButton', 'pulsante A', 'premuto'],
      ...
      ['-'],
      ['h', 'quando pin %n 猫 %m.outputs', 'whenDigitalRead', 1, 'acceso'],
    ...
    ja: [
      ['h', '?囥??ゃ偣?屻仱??????ㄣ?', 'whenConnected'],
      [' ', '%m.hwOut ??%n ?斻兂?搞仱??', 'connectHW', 'led A', 3],
      [' ', '%m.hwIn ?掋偄?娿儹?板叆??%n ?斻兂?搞仱??', 'connectHW', 'rotation knob', 0],
      ['-'],
      [' ', '%m.leds ??%m.outputs ????, 'digitalLED', 'led A', 'on'],
      [' ', '%m.leds ???嬨???%n% ????, 'setLED', 'led A', 100],
      [' ', '%m.leds ???嬨???%n% ?氥仱澶夈???, 'changeLED', 'led A', 20],
      ['-'],
      [' ', '%m.servos ??%n 搴︺伕?戙???, 'rotateServo', 'servo A', 180],
      [' ', '%m.servos ??%n 搴︺??ゅ???, 'changeServo', 'servo A', 20],
      ['-'],
      ['h', '%m.buttons ??%m.btnStates ?ㄣ?', 'whenButton', '?溿偪??A', '?笺??屻?'],
      ['b', '%m.buttons ?笺??屻?', 'isButtonPressed', '?溿偪??A'],
      ['-'],
      ['h', '%m.hwIn ??%m.ops %n% ?仾????ㄣ?', 'whenInput', '?炶虎?ゃ伨??, '>', 50],
      ['r', '%m.hwIn ?�?, 'readInput', '?炶虎?ゃ伨??],
      ['-'],
      [' ', '?囥偢?裤儷?哄? %n ??%m.outputs ????, 'digitalWrite', 1, 'on'],
      [' ', '?€??偘?哄? %n ??%n% ????, 'analogWrite', 3, 100],
      ['-'],
      ['h', '?囥偢?裤儷?ュ? %n ??%m.outputs ?仾????ㄣ?', 'whenDigitalRead', 1, 'on'],
      ['b', '?囥偢?裤儷?ュ? %n ?偑??, 'digitalRead', 1],
      ['-'],
      ['h', '?€??偘?ュ? %n ??%m.ops %n% ?仾????ㄣ?', 'whenAnalogRead', 1, '>', 50],
      ['r', '?€??偘?ュ? %n ?�?, 'analogRead', 0],
      ['-'],
      ['r', '%n ??%n ... %n ?嬨? %n ... %n ?稿???, 'mapValues', 50, 0, 100, -240, 240]
    ],
    nb: [
      ['h', 'n氓r enheten tilkobles', 'whenConnected'],
      [' ', 'koble %m.hwOut til digital %n', 'connectHW', 'LED A', 3],
      [' ', 'koble %m.hwIn til analog %n', 'connectHW', 'dreieknapp', 0],
      ['-'],
      [' ', 'sett %m.leds %m.outputs', 'digitalLED', 'LED A', 'p氓'],
      [' ', 'sett %m.leds styrke til %n%', 'setLED', 'LED A', 100],
      [' ', 'endre %m.leds styrke med %n%', 'changeLED', 'LED A', 20],
      ['-'],
      [' ', 'rot茅r %m.servos til %n grader', 'rotateServo', 'servo A', 180],
      [' ', 'rot茅r %m.servos med %n grader', 'changeServo', 'servo A', 20],
      ['-'],
      ['h', 'n氓r %m.buttons %m.btnStates', 'whenButton', 'knapp A', 'trykkes'],
      ['b', '%m.buttons trykket?', 'isButtonPressed', 'knapp A'],
      ['-'],
      ['h', 'n氓r %m.hwIn %m.ops %n%', 'whenInput', 'dreieknapp', '>', 50],
      ['r', '%m.hwIn verdi', 'readInput', 'dreieknapp'],
      ['-'],
      [' ', 'sett digital %n %m.outputs', 'digitalWrite', 1, 'p氓'],
      [' ', 'set utgang %n til %n%', 'analogWrite', 3, 100],
      ['-'],
      ['h', 'n氓r digital %n er %m.outputs', 'whenDigitalRead', 1, 'p氓'],
      ['b', 'digital %n p氓?', 'digitalRead', 1],
      ['-'],
      ['h', 'n氓r analog %n %m.ops %n%', 'whenAnalogRead', 1, '>', 50],
      ['r', 'analog %n verdi', 'analogRead', 0],
      ['-'],
      ['r', 'skal茅r %n fra %n %n til %n %n', 'mapValues', 50, 0, 100, -240, 240]
    ],
    nl: [
    ...
    el: [
      ['h', '??伪谓 畏 ???魏蔚?萎 蔚委谓伪喂 ??谓未蔚未蔚渭苇谓畏', 'whenConnected'],
      [' ', '??谓未蔚?蔚 ?慰 %m.hwOut ??慰 pin %n', 'connectHW', 'led A', 3],
      [' ', '??谓未蔚?蔚 ?慰 %m.hwIn ??慰 伪谓伪位慰纬喂魏? %n', 'connectHW', '?慰谓?蔚?喂?渭蔚??慰', 0],
      ['-'],
      [' ', '维位位伪尉蔚 ?慰 %m.leds ?蔚 %m.outputs', 'digitalLED', 'led A', '蔚谓蔚?纬慰?慰喂畏渭苇谓慰'],
      [' ', '??喂?蔚 ??慰 %m.leds ?畏 ???蔚喂谓??畏?伪 委?畏 渭蔚 %n%', 'setLED', 'led A', 100],
      [' ', '维位位伪尉蔚 ??慰 %m.leds ?畏 ???蔚喂谓??畏?伪 魏伪?维 %n%', 'changeLED', 'led A', 20],
      ['-'],
      [' ', '???委?蔚 ?慰 %m.servos ??喂? %n 渭慰委?蔚?', 'rotateServo', 'servo A', 180],
      [' ', '???委?蔚 ?慰 %m.servos 魏伪?维 %n 渭慰委?蔚?', 'changeServo', 'servo A', 20],
      ['-'],
      ['h', '??伪谓 ?慰 %m.buttons 蔚委谓伪喂 %m.btnStates', 'whenButton', '魏慰?渭?委 A', '?伪?畏渭苇谓慰'],
      ['b', '?慰 %m.buttons ?伪?萎胃畏魏蔚;', 'isButtonPressed', '魏慰?渭?委 A'],
      ['-'],
      ['h', '??伪谓 ?慰 %m.hwIn %m.ops %n%', 'whenInput', '?慰谓?蔚?喂?渭蔚??慰', '>', 50],
      ['r', '未喂维尾伪?蔚 %m.hwIn', 'readInput', '?慰谓?蔚?喂?渭蔚??慰'],
      ['-'],
      [' ', '维位位伪尉蔚 ?慰 pin %n ?蔚 %m.outputs', 'digitalWrite', 1, '蔚谓蔚?纬慰?慰喂畏渭苇谓慰'],
      [' ', '??喂?蔚 ?慰 pin %n ?蔚 %n%', 'analogWrite', 3, 100],
      ['-'],
      ['h', '??伪谓 ?慰 pin %n 蔚委谓伪喂 %m.outputs', 'whenDigitalRead', 1, '蔚谓蔚?纬慰?慰喂畏渭苇谓慰'],
      ['b', '?慰 pin %n 蔚委谓伪喂 蔚谓蔚?纬慰?慰喂畏渭苇谓慰;', 'digitalRead', 1],
      ['-'],
      ['h', '??伪谓 ?慰 伪谓伪位慰纬喂魏? %n %m.ops %n%', 'whenAnalogRead', 1, '>', 50],
      ['r', '未喂维尾伪?蔚 ?慰 伪谓伪位慰纬喂魏? %n', 'analogRead', 0],
      ['-'],
      ['r', '????苇?喂?蔚 %n 伪?? %n %n 苇?? %n %n', 'mapValues', 50, 0, 100, -240, 240]
    ],
    es: [
      ['h', 'al conectar el dispositivo', 'whenConnected'],
      [' ', 'conectar %m.hwOut al pin %n', 'connectHW', 'led A', 3], 
      [' ', 'conectar %m.hwIn al pin anal贸gico %n', 'connectHW', 'potenci贸metro', 0], 
      ['-'],
      ...
      ['-'],
      [' ', 'apuntar %m.servos en direcci贸n %n grados', 'rotateServo', 'servo A', 180],
      [' ', 'girar %m.servos %n grados', 'changeServo', 'servo A', 20],
      ['-'],
      ['h', 'cuando el %m.buttons est茅 %m.btnStates', 'whenButton', 'bot贸n A', 'presionado'],
      ['b', '驴%m.buttons presionado?', 'isButtonPressed', 'bot贸n A'],
      ['-'],
      ['h', 'cuando %m.hwIn %m.ops %n%', 'whenInput', 'potenci贸metro', '>', 50],
      ['r', 'leer %m.hwIn', 'readInput', 'potenci贸metro'],
      ['-'],
      [' ', 'fijar estado de pin %n a %m.outputs', 'digitalWrite', 1, 'on'],
      [' ', 'fijar pin anal贸gico %n al %n%', 'analogWrite', 3, 100],
      ['-'],
      ['h', 'cuando el pin %n est茅 %m.outputs', 'whenDigitalRead', 1, 'on'],
      ['b', '驴pin %n on?', 'digitalRead', 1], 
      ['-'],
      ['h', 'cuando pin anal贸gico %n %m.ops %n%', 'whenAnalogRead', 1, '>', 50],
      ['r', 'leer anal贸gico %n', 'analogRead', 0], 
      ['-'],
  ...
  var menus = {
    ...
    de: {
      ...
      btnStates: ['gedr眉ckt', 'losgelassen'],
    ...
    ja: {
      buttons: ['?溿偪??A', '?溿偪??B', '?溿偪??C', '?溿偪??D'],
      btnStates: ['?笺??屻?', '?俱??屻?'],
      hwIn: ['?炶虎?ゃ伨??, '?夈偦?炽偟??, '娓╁害?汇兂?点兗'],
      hwOut: ['led A', 'led B', 'led C', 'led D', '?溿偪??A', '?溿偪??B', '?溿偪??C', '?溿偪??D', '?点兗??A', '?点兗??B', '?点兗??C', '?点兗??D'],
      leds: ['led A', 'led B', 'led C', 'led D'],
      outputs: ['?兂', '??'],
      ops: ['>', '=', '<'],
      servos: ['?点兗??A', '?点兗??B', '?点兗??C', '?点兗??D']
    },
    nb: {
    ...
    el: {
      buttons: ['魏慰?渭?委 A', '魏慰?渭?委 B', '魏慰?渭?委 C', '魏慰?渭?委 D'],
      btnStates: ['?伪?畏渭苇谓慰', '蔚位蔚?胃蔚?慰'],
      hwIn: ['?慰谓?蔚?喂?渭蔚??慰', '???慰伪喂?胃畏?萎?伪', '胃蔚?渭慰伪喂?胃畏?萎?伪'],
      hwOut: ['led A', 'led B', 'led C', 'led D', '魏慰?渭?委 A', '魏慰?渭?委 B', '魏慰?渭?委 C', '魏慰?渭?委 D', 'servo A', 'servo B', 'servo C', 'servo D'],
      leds: ['led A', 'led B', 'led C', 'led D'],
      outputs: ['蔚谓蔚?纬慰?慰喂畏渭苇谓慰', '伪?蔚谓蔚?纬慰?慰喂畏渭苇谓慰'],
      ops: ['>', '=', '<'],
      servos: ['servo A', 'servo B', 'servo C', 'servo D']
    },
    es: {
      buttons: ['bot贸n A', 'bot贸n B', 'bot贸n C', 'bot贸n D'],
      btnStates: ['pulsado', 'liberado'],
      hwIn: ['potenci贸metro', 'sensor de luz', 'sensor de temperatura'],
      hwOut: ['led A', 'led B', 'led C', 'led D', 'bot贸n A', 'bot贸n B', 'bot贸n C', 'bot贸n D', 'servo A', 'servo B', 'servo C', 'servo D'],
      leds: ['led A', 'led B', 'led C', 'led D'],
      outputs: ['on', 'off'],
      ops: ['>', '=', '<'],
      servos: ['servo A', 'servo B', 'servo C', 'servo D']
