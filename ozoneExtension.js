...
(function(ext) {
  ...
  var blocks = {
    ...
    ja: [
      ['h', '?????ƒå£?å±»ä»±?????????', 'whenConnected'],
      [' ', '%m.hwOut ??%n ??»å???žä»±???', 'connectHW', 'led A', 3],
      [' ', '%m.hwIn ??‹å??å¨¿å„¹??¿å???%n ??»å???žä»±???', 'connectHW', 'rotation knob', 0],
      ['-'],
      [' ', '%m.leds ??%m.outputs ?????, 'digitalLED', 'led A', 'on'],
      [' ', '%m.leds ????å¬???%n% ?????, 'setLED', 'led A', 100],
      [' ', '%m.leds ????å¬???%n% ?æ°¥ä»±æ¾¶å????, 'changeLED', 'led A', 20],
      ['-'],
      [' ', '%m.servos ??%n ?´ï¸ºä¼?????, 'rotateServo', 'servo A', 180],
      [' ', '%m.servos ??%n ?´ï¸º??????, 'changeServo', 'servo A', 20],
      ['-'],
      ['h', '%m.buttons ??%m.btnStates ???', 'whenButton', '?æº¿åª??A', '?ç¬??å±?'],
      ['b', '%m.buttons ?ç¬??å±?', 'isButtonPressed', '?æº¿åª??A'],
      ['-'],
      ['h', '%m.hwIn ??%m.ops %n% ?? ä»¾??????', 'whenInput', '??¶è???ƒä¼¨??, '>', 50],
      ['r', '%m.hwIn ??¼ï¿½?, 'readInput', '??¶è???ƒä¼¨??],
      ['-'],
      [' ', '??¥å¢?è£¤å„·??? %n ??%m.outputs ?????, 'digitalWrite', 1, 'on'],
      [' ', '?????œå???? %n ??%n% ?????, 'analogWrite', 3, 100],
      ['-'],
      ['h', '??¥å¢?è£¤å„·??? %n ??%m.outputs ?? ä»¾??????', 'whenDigitalRead', 1, 'on'],
      ['b', '??¥å¢?è£¤å„·??? %n ??˜å???, 'digitalRead', 1],
      ['-'],
      ['h', '?????œå???? %n ??%m.ops %n% ?? ä»¾??????', 'whenAnalogRead', 1, '>', 50],
      ['r', '?????œå???? %n ??¼ï¿½?, 'analogRead', 0],
      ['-'],
      ['r', '%n ??%n ... %n ?å¬? %n ... %n ?ç¨???, 'mapValues', 50, 0, 100, -240, 240]
    ],
    nb: [
      ['h', 'næ°“r enheten tilkobles', 'whenConnected'],
      [' ', 'koble %m.hwOut til digital %n', 'connectHW', 'LED A', 3],
      [' ', 'koble %m.hwIn til analog %n', 'connectHW', 'dreieknapp', 0],
      ['-'],
      [' ', 'sett %m.leds %m.outputs', 'digitalLED', 'LED A', 'pæ°?],
      ...
      ['-'],
      ['h', 'næ°“r %m.buttons %m.btnStates', 'whenButton', 'knapp A', 'trykkes'],
      ['b', '%m.buttons trykket?', 'isButtonPressed', 'knapp A'],
      ['-'],
      ['h', 'næ°“r %m.hwIn %m.ops %n%', 'whenInput', 'dreieknapp', '>', 50],
      ['r', '%m.hwIn verdi', 'readInput', 'dreieknapp'],
      ['-'],
      [' ', 'sett digital %n %m.outputs', 'digitalWrite', 1, 'pæ°?],
      [' ', 'set utgang %n til %n%', 'analogWrite', 3, 100],
      ['-'],
      ['h', 'næ°“r digital %n er %m.outputs', 'whenDigitalRead', 1, 'pæ°?],
      ['b', 'digital %n pæ°?', 'digitalRead', 1],
      ['-'],
      ['h', 'næ°“r analog %n %m.ops %n%', 'whenAnalogRead', 1, '>', 50],
    ...
    el: [
      ['h', '??ä¼ªè? ?????é­è?????šå?è°“ä¼ª????è°“æœª?šæœª?šæ¸­?‡è???, 'whenConnected'],
      [' ', '??è°“æœª???????%m.hwOut ????pin %n', 'connectHW', 'led A', 3],
      [' ', '??è°“æœª???????%m.hwIn ????ä¼ªè?ä¼ªä??°çº¬?‚é?? %n', 'connectHW', '??°è??????æ¸­è?????, 0],
      ['-'],
      [' ', 'ç»´ä?ä½ä¼ªå°‰è? ???%m.leds ???%m.outputs', 'digitalLED', 'led A', '?šè???çº¬æ…°??°å??æ¸­?‡è???],
      [' ', '??????????%m.leds ???????šå?è°????ä¼?å§???æ¸­è? %n%', 'setLED', 'led A', 100],
      [' ', 'ç»´ä?ä½ä¼ªå°‰è? ????%m.leds ???????šå?è°????ä¼?é­ä¼ª?ç»?%n%', 'changeLED', 'led A', 20],
      ['-'],
      [' ', '???å§??????%m.servos ???? %n æ¸­æ…°å§???', 'rotateServo', 'servo A', 180],
      [' ', '???å§??????%m.servos é­ä¼ª?ç»?%n æ¸­æ…°å§???', 'changeServo', 'servo A', 20],
      ['-'],
      ['h', '??ä¼ªè? ???%m.buttons ?šå?è°“ä¼ª??%m.btnStates', 'whenButton', 'é­æ…°?æ¸?å§?A', '?ä¼??æ¸­?‡è???],
      ['b', '???%m.buttons ?ä¼??Žè??é???', 'isButtonPressed', 'é­æ…°?æ¸?å§?A'],
      ['-'],
      ['h', '??ä¼ªè? ???%m.hwIn %m.ops %n%', 'whenInput', '??°è??????æ¸­è?????, '>', 50],
      ['r', '?ªå?ç»´å°¾ä¼???%m.hwIn', 'readInput', '??°è??????æ¸­è?????],
      ['-'],
      [' ', 'ç»´ä?ä½ä¼ªå°‰è? ???pin %n ???%m.outputs', 'digitalWrite', 1, '?šè???çº¬æ…°??°å??æ¸­?‡è???],
      [' ', '?????????pin %n ???%n%', 'analogWrite', 3, 100],
      ['-'],
      ['h', '??ä¼ªè? ???pin %n ?šå?è°“ä¼ª??%m.outputs', 'whenDigitalRead', 1, '?šè???çº¬æ…°??°å??æ¸­?‡è???],
      ['b', '???pin %n ?šå?è°“ä¼ª???šè???çº¬æ…°??°å??æ¸­?‡è???', 'digitalRead', 1],
      ['-'],
      ['h', '??ä¼ªè? ???ä¼ªè?ä¼ªä??°çº¬?‚é?? %n %m.ops %n%', 'whenAnalogRead', 1, '>', 50],
      ['r', '?ªå?ç»´å°¾ä¼??????ä¼ªè?ä¼ªä??°çº¬?‚é?? %n', 'analogRead', 0],
      ['-'],
      ['r', '??????????%n ä¼?? %n %n ??? %n %n', 'mapValues', 50, 0, 100, -240, 240]
    ],
    es: [
      ['h', 'al conectar el dispositivo', 'whenConnected'],
      [' ', 'conectar %m.hwOut al pin %n', 'connectHW', 'led A', 3], 
      [' ', 'conectar %m.hwIn al pin analè´¸gico %n', 'connectHW', 'potenciè´¸metro', 0], 
      ['-'],
      ...
      ['-'],
      [' ', 'apuntar %m.servos en direcciè´¸n %n grados', 'rotateServo', 'servo A', 180],
      [' ', 'girar %m.servos %n grados', 'changeServo', 'servo A', 20],
      ['-'],
      ['h', 'cuando el %m.buttons est??%m.btnStates', 'whenButton', 'botè´¸n A', 'presionado'],
      ['b', 'é©?m.buttons presionado?', 'isButtonPressed', 'botè´¸n A'],
      ['-'],
      ['h', 'cuando %m.hwIn %m.ops %n%', 'whenInput', 'potenciè´¸metro', '>', 50],
      ['r', 'leer %m.hwIn', 'readInput', 'potenciè´¸metro'],
      ['-'],
      [' ', 'fijar estado de pin %n a %m.outputs', 'digitalWrite', 1, 'on'],
      [' ', 'fijar pin analè´¸gico %n al %n%', 'analogWrite', 3, 100],
      ['-'],
      ['h', 'cuando el pin %n est??%m.outputs', 'whenDigitalRead', 1, 'on'],
      ['b', 'é©´pin %n on?', 'digitalRead', 1], 
      ['-'],
      ['h', 'cuando pin analè´¸gico %n %m.ops %n%', 'whenAnalogRead', 1, '>', 50],
      ['r', 'leer analè´¸gico %n', 'analogRead', 0], 
      ['-'],
  ...
  var menus = {
    ...
    ja: {
      buttons: ['?æº¿åª??A', '?æº¿åª??B', '?æº¿åª??C', '?æº¿åª??D'],
      btnStates: ['?ç¬??å±?', '?ä¿??å±?'],
      hwIn: ['??¶è???ƒä¼¨??, '?å¤ˆå¦??½å???, 'å¨“â?å®?æ±‡å???¹å?'],
      hwOut: ['led A', 'led B', 'led C', 'led D', '?æº¿åª??A', '?æº¿åª??B', '?æº¿åª??C', '?æº¿åª??D', '??¹å???A', '??¹å???B', '??¹å???C', '??¹å???D'],
      leds: ['led A', 'led B', 'led C', 'led D'],
      outputs: ['??‚å?', '???'],
      ops: ['>', '=', '<'],
      servos: ['??¹å???A', '??¹å???B', '??¹å???C', '??¹å???D']
    },
    nb: {
    ...
    el: {
      buttons: ['é­æ…°?æ¸?å§?A', 'é­æ…°?æ¸?å§?B', 'é­æ…°?æ¸?å§?C', 'é­æ…°?æ¸?å§?D'],
      btnStates: ['?ä¼??æ¸­?‡è???, '?šä????ƒè????],
      hwIn: ['??°è??????æ¸­è?????, '????°ä¼ª???ƒç????ä¼?, '?ƒè??æ¸­æ…°ä¼ªå???ƒç????ä¼?],
      hwOut: ['led A', 'led B', 'led C', 'led D', 'é­æ…°?æ¸?å§?A', 'é­æ…°?æ¸?å§?B', 'é­æ…°?æ¸?å§?C', 'é­æ…°?æ¸?å§?D', 'servo A', 'servo B', 'servo C', 'servo D'],
      leds: ['led A', 'led B', 'led C', 'led D'],
      outputs: ['?šè???çº¬æ…°??°å??æ¸­?‡è???, 'ä¼??šè???çº¬æ…°??°å??æ¸­?‡è???],
      ops: ['>', '=', '<'],
      servos: ['servo A', 'servo B', 'servo C', 'servo D']
    },
    es: {
      buttons: ['botè´¸n A', 'botè´¸n B', 'botè´¸n C', 'botè´¸n D'],
      btnStates: ['pulsado', 'liberado'],
      hwIn: ['potenciè´¸metro', 'sensor de luz', 'sensor de temperatura'],
      hwOut: ['led A', 'led B', 'led C', 'led D', 'botè´¸n A', 'botè´¸n B', 'botè´¸n C', 'botè´¸n D', 'servo A', 'servo B', 'servo C', 'servo D'],
      leds: ['led A', 'led B', 'led C', 'led D'],
      outputs: ['on', 'off'],
      ops: ['>', '=', '<'],
      servos: ['servo A', 'servo B', 'servo C', 'servo D']
